...
void shoot_init(void)
{
    ...
    //拨叉电机指针（2006）
    shoot_system.push_wheel_motor.motor_measure = get_Push_Motor_Measure_Point();
		//拨轮电机指针（6020）
    ...
    shoot_system.shoot_motor.speed_set = 0.0f;
		...
		shoot_system.push_wheel_motor.ecd_count = 0;
		shoot_system.push_wheel_motor.given_current = 0;
		shoot_system.push_wheel_motor.speed = 0.0f;
		shoot_system.push_wheel_motor.speed_set = 0.0f;
...
//////////////6020 拨轮电机
		shoot_system.push_wheel_motor.speed = (shoot_system.push_wheel_motor.motor_measure->speed_rpm * PUSH_Motor_RMP_TO_SPEED);
		//微动开关
...
//			&& shoot_system.shoot_motor.move_flag == 0)
	...
	{
			shoot_system.push_wheel_motor.speed_set = PUSH_SPEED;	
			shoot_system.shoot_motor.speed_set = 0.0f;
	}
	...
	{
			if (rad_format(shoot_system.shoot_motor.set_angle - shoot_system.shoot_motor.angle) > 0.05f)
			{
					//没到达,因此一直设置旋转速度
					shoot_system.shoot_motor.speed_set = SHOOT_SPEED;					
			}
...
//				}
        //计算拨弹轮电机PID
        PID_Calc(&push_motor_pid, shoot_system.push_wheel_motor.speed, shoot_system.push_wheel_motor.speed_set);
        PID_Calc(&shoot_motor_pid, shoot_system.shoot_motor.speed, shoot_system.shoot_motor.speed_set);
        shoot_system.push_wheel_motor.given_current = (int16_t)(push_motor_pid.out);
...
#if PUSH_MOTOR_TURN
				Push_Can_Set_Current  = -shoot_system.push_wheel_motor.given_current;
#else
				Push_Can_Set_Current  = shoot_system.push_wheel_motor.given_current;
#endif
